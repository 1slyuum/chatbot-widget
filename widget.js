...
(function () {
  ...
  // data-lang sets the BCP-47 locale used for time formatting & is applied
  // as the `lang` attribute. data-dir = "auto" | "ltr" | "rtl".
  // "auto" detects RTL from the locale and/or the configured text content.
  // ───────────────────────────────────────────────
  var RTL_LANGS = /^(ar|he|fa|ur|ps|sd|ug|yi|dv|ckb|arc|nqo)\b/i;
  // Unicode ranges for RTL scripts (Arabic, Hebrew, Syriac, Thaana, NKo, etc.)
  var RTL_CHAR = /[\u0590-\u05FF\u0600-\u06FF\u0700-\u074F\u0750-\u077F\u0780-\u07BF\u07C0-\u07FF\u08A0-\u08FF\uFB1D-\uFDFF\uFE70-\uFEFF]/;

  var langSetting = attr('data-lang', '');
  // Fall back to the page's lang, then the browser's language.
  ...
  if (dirSetting === 'rtl') {
    ...
    // auto: locale hint OR RTL characters in the business name / configured strings.
  ...
  // First "grapheme" of a string — Unicode-aware so non-Latin scripts and
  // emoji produce a sensible avatar letter (e.g. Arabic, Chinese, emoji flags).
  function firstGrapheme(str) {
    ...
    // Fallback: handle surrogate pairs (emoji) without breaking them.
    var cp = str.codePointAt(0);
    return String.fromCodePoint(cp);
  }

  // Uppercase only makes sense for cased scripts; toLocaleUpperCase is safe
  // for others (it's a no-op for Arabic/CJK).
  ...
  var CONFIG = {
    ...
    // Localizable UI strings (everything the widget renders itself).
    textToday: attr('data-text-today', ''),         // divider label; '' = auto via Intl
  ...
  #cw-greeting {
    position: fixed;
    bottom: calc(${CONFIG.offsetY + LSIZE + 12}px + env(safe-area-inset-bottom, 0px));
    ${SIDE}: calc(${CONFIG.offsetX}px + env(safe-area-inset-${SIDE}, 0px));
    max-width: 240px;
  ...
  /* Phones / small screens: take (almost) the full viewport for comfortable typing.
     A JS-managed --cw-vh variable tracks the *visible* viewport so the layout
     follows the on-screen keyboard on iOS/Android. */
  @media (max-width: 560px) {
    ...
    /* Hide the launcher while the full-screen panel is open on mobile. */
    #cw-launcher.open { opacity: 0; pointer-events: none; transform: scale(0.6); }
  ...
  #cw-widget[dir="rtl"] .cw-bubble.ai { border-bottom-left-radius: var(--cw-radius-sm); border-bottom-right-radius: 4px; }
  #cw-widget[dir="rtl"] .cw-bubble.user { border-bottom-right-radius: var(--cw-radius-sm); border-bottom-left-radius: 4px; }
  ...
  #cw-widget[dir="rtl"] .cw-bubble.ai blockquote { border-right: 3px solid ${CONFIG.primaryColor}66; border-left: none; padding: 2px 10px 2px 0; }
  #cw-widget[dir="rtl"] .cw-send-icon { transform: scaleX(-1); }
  /* Per-message direction: bubbles auto-align their own text via dir="auto". */
  .cw-bubble { unicode-bidi: plaintext; }
  ...
  @media (max-width: 560px) {
    ...
    .cw-send-btn { width: 40px; height: 40px; }
  ...
  var rootHtml = `
  <button id="cw-launcher" aria-label="${escapeHtml(CONFIG.textOpenAria)}" aria-expanded="false" aria-controls="cw-widget">
  ...
  <div id="cw-widget" role="dialog" lang="${escapeHtml(CONFIG.lang)}" dir="${CONFIG.dir}" aria-label="${escapeHtml(CONFIG.businessName)}" aria-modal="false">
    <div class="cw-header">
      ${avatarMarkup()}
      ...
      <div class="cw-header-actions">
        <button class="cw-icon-btn" id="cw-clear-btn" title="${escapeHtml(CONFIG.textNewChat)}" aria-label="${escapeHtml(CONFIG.textNewChat)}">
          <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
            <path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/>
          </svg>
        </button>
        ${CONFIG.headerBadge ? '<div class="cw-header-badge">' + escapeHtml(CONFIG.headerBadge) + '</div>' : ''}
        <button class="cw-icon-btn" id="cw-close-btn" title="${escapeHtml(CONFIG.textClose)}" aria-label="${escapeHtml(CONFIG.textClose)}">
    ...
    <div class="cw-input-bar">
      <div class="cw-input-wrap">
        <textarea class="cw-textarea" id="cw-input" placeholder="${escapeHtml(CONFIG.placeholder)}" rows="1" aria-label="${escapeHtml(CONFIG.placeholder)}" enterkeyhint="send" autocapitalize="sentences" autocomplete="off"></textarea>
        <button class="cw-send-btn" id="cw-send-btn" title="${escapeHtml(CONFIG.textSend)}" aria-label="${escapeHtml(CONFIG.textSend)}">
          <svg class="cw-send-icon" width="16" height="16" fill="none" stroke="${iconColor}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
            <line x1="22" y1="2" x2="11" y2="13"/>
            <polygon points="22 2 15 22 11 13 2 9 22 2"/>
          </svg>
        </button>
      </div>
      ${CONFIG.showPoweredBy ? '<div class="cw-input-footer">' + escapeHtml(CONFIG.textPoweredBy) + ' ' + poweredHtml + '</div>' : ''}
    </div>
  </div>
  `;
  ...
  function boot() {
    ...
    // Track the visible viewport height so the mobile layout follows the
    // on-screen keyboard (iOS Safari / Android Chrome).
  ...
  function init(root, syncViewportHeight) {
    ...
    // Locale-aware "Today" label for the date divider.
    function todayLabel() {
      if (CONFIG.textToday) return CONFIG.textToday;
      try {
        if (typeof Intl !== 'undefined' && Intl.RelativeTimeFormat) {
          var rtf = new Intl.RelativeTimeFormat(CONFIG.lang, { numeric: 'auto' });
          // "today" / "اليوم" / "今天" etc.
    ...
    function lockBodyScroll(lock) {
      // Only lock the page behind the full-screen panel on mobile.
      if (!isMobile()) return;
    ...
    function setOpen(open) {
      ...
      if (isOpen) {
        ...
        hideGreeting();
        lockBodyScroll(true);
        // On mobile, focusing immediately can fight the open animation; wait a tick.
        setTimeout(function () { try { input.focus({ preventScroll: true }); } catch (e) { input.focus(); } }, isMobile() ? 350 : 400);
        scrollToBottom();
      } else {
    ...
    function sendMessage() {
      ...
      fetch(CONFIG.webhookUrl, {
        ...
        if (reply && String(reply).trim()) {
          ...
          appendMessage(CONFIG.errEmpty, 'ai');
        }
